...
write(lines, file = "test.txt", sep = "\t")

dat_RIVM_test <- read.table("test.txt", skip = start, nrows = stop - start, fill = TRUE, colClasses = "numeric")
colnames(dat_RIVM_test) <- c("week", "totaal", "positief", "prop_pos")
dat_RIVM_test$week <- 23:(23 + dim(dat_RIVM_test)[1] - 1)
